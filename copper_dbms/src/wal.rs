...
/*
A WAL is a Write Ahead Log used to save the operations done on the database into the disk in case we lost the memtable.
Thus we need to implement a way of writting into it, loading the previous memtable.
*/

pub fn write_to_wal(database_path: &Path, operation: &Operation) -> Result<(), &str> {
    // Get the WAL path
    let wal_path = database_path.join("wal.txt");
